...
/*   Updated: 2025/02/26 16:10:55 by albelaiz         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_link(t_game *game)
{
	...
	while (game->map[i] != NULL)
	{
		...
		{
			if (game->map[i][j] == '1')
				mlx_put_image_to_window(game->mlx, game->mlx_win, game->wall, x,
					y);
			else if (game->map[i][j] == 'P')
			{
				...
				mlx_put_image_to_window(game->mlx, game->mlx_win, game->exit, x,
					y);
			j++;
			x += 64;
		}
